...
io.on("connection", (socket) => {
	...
	socket.on("chat", (room, msg) => {
		queryDB("projects", (collection) =>
			collection.updateOne({ _id: room }, { $push: { chat: msg } })
		)
			.then((res) =>
				res.result.n === 1
					? io.to(room).emit("rcv_msg", room, msg)
					: io
							.to(room)
							.emit("rcv_msg", room, { message: "err", sender: "node" })
			)
			.catch((err) =>
				io.to(room).emit("rcv_msg", room, { message: "err", sender: "node" })
			);
	});
});
